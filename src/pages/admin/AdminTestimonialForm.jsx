...
import { 
  ...
  User, MapPin, BookOpen, MessageSquare, Globe
} from "lucide-react";
...
export default function AdminTestimonialForm() {
  ...
  useEffect(() => {
    ...
    if (isEditing) fetchTestimonial();
  }, [hasToken, isEditing]);

  const fetchTestimonial = async () => {
    try {
      const res = await adminApi.get(`/api/admin/testimonials/${id}`);
      const data = res.data;
      setForm({
        name: data.name || "",
        place: data.place || "",
      ...
      setError(e?.response?.data?.message || "Failed to load testimonial");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await adminApi.put(`/api/admin/testimonials/${id}`, form);
        setSuccess("Testimonial updated successfully!");
      } else {
        await adminApi.post("/api/admin/testimonials", form);
  ...
  if (loading) {
    return (
      <AdminLayout title={isEditing ? "Edit Testimonial" : "Add Testimonial"} subtitle="Loading...">
  ...
  return (
    <AdminLayout 
      ...
      subtitle={isEditing ? "Update student feedback" : "Create new student feedback"}
    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows={4}
                className="mt-1.5 w-full rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#C62828]/20 focus:border-[#C62828] transition resize-none"
                placeholder="Student feedback message..."
                required
              />
            </div>
