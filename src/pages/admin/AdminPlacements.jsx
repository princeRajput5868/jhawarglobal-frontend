...
import { 
  ...
  User, Briefcase, Building, Loader2, AlertCircle, CheckCircle, XCircle
} from "lucide-react";
...
export default function AdminPlacements() {
  ...
  const fetchPlacements = async () => {
    ...
    setError(null);
    try {
      const res = await adminApi.get("/api/admin/placements");
      setPlacements(Array.isArray(res.data) ? res.data : []);
    } catch (e) {
      setError(e?.response?.data?.message || "Failed to load placements");
    } finally {
      setLoading(false);
    }
  };

  const onDelete = async (placement) => {
    const ok = window.confirm(`Delete placement for "${placement.name}"?`);
    if (!ok) return;
    setDeletingId(placement.id);
    try {
      await adminApi.delete(`/api/admin/placements/${placement.id}`);
      fetchPlacements();
    } catch (e) {
  ...
  return (
    <AdminLayout 
      title="Placements" 
      subtitle="Manage student success stories"
      actions={
        <Link
    ...
    >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex items-center justify-between">
          <span className="font-semibold text-gray-700">{filtered.length} placements found</span>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr className="text-left">
                ...
                <th className="p-4 font-bold text-gray-500 uppercase tracking-wider text-[10px]">Role</th>
                <th className="p-4 font-bold text-gray-500 uppercase tracking-wider text-[10px]">Company</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="p-8 text-center text-gray-500">
                    No placements found. Add your first success story!
                  </td>
                </tr>
              ) : (
                filtered.map((p) => (
